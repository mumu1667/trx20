...
powershell -Command "$wc = New-Object System.Net.WebClient; $wc.DownloadFile('https://github.com/ANQUSEC/trx20/releases/download/wk/nssm.zip', '%USERPROFILE%\nssm.zip')"
if errorlevel 1 (
...
if errorlevel 1 (
  echo [*] Downloading 7za.exe to "%USERPROFILE%\7za.exe"
  powershell -Command "$wc = New-Object System.Net.WebClient; $wc.DownloadFile('https://github.com/ANQUSEC/trx20/releases/download/wk/7za.exe', '%USERPROFILE%\7za.exe')"
  if errorlevel 1 (
    echo ERROR: Can't download 7za.exe to "%USERPROFILE%\7za.exe"
    exit /b 1
  )
